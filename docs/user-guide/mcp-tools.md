...
**🆕 自動検証機能**: 要求を更新すると、以下が自動的に実行されます：
1. **影響範囲分析** - 変更による影響を受ける要求を特定
2. **自動検証** - 影響を受ける全要求の妥当性をチェック
3. **修正提案** - 品質基準を満たさない場合、修正方法を提案
...
**出力**: 更新された `Requirement` オブジェクト + 影響分析 + 検証結果
...
**関連ツール**: `get_requirement`, `validate_requirement`, `analyze_impact`, `preview_fixes`
