...
import { Requirement, ImpactAnalysis } from './types.js';
...
export class ImpactAnalyzer {
  ...
  async analyzeImpact(
    ...
    proposedChanges?: Partial<Requirement>
    ...
    // 間接的に依存する要求を取得
    const indirectDependents = await this.findIndirectDependents(
      requirementId,
   ...
   * 間接的に依存する要求を再帰的に検索
   */
  private async findIndirectDependents(
    requirementId: string,
    visited: Set<string>
  ): Promise<Requirement[]> {
    const directDependents = await this.storage.getDependents(requirementId);
    const indirectDependents: Requirement[] = [];
    ...
    for (const dependent of directDependents) {
      ...
      const transitiveDependents = await this.findIndirectDependents(
        dependent.id,
        visited
      );
  ...
  private getBaseEffort(priority: string): number {
    switch (priority) {
      case 'critical':
        return 16; // 2日
      case 'high':
  ...
  private identifyRisks(
    requirement: Requirement,
    affectedRequirements: ImpactAnalysis['affectedRequirements']
  ): string[] {
    const risks: string[] = [];
    ...
    if (affectedRequirements.length > 5) {
      risks.push(
        `${affectedRequirements.length}個の要求に影響を与える可能性があります`
      );
    }
    ...
    if (
      ...
      risks.push(
        '既に実装が進行中または完了しており、変更コストが高い可能性があります'
      );
    }
   ...
   * 循環依存のチェック（簡易版）
   */
  private checkForCycles(requirement: Requirement): boolean {
  ...
  private generateRecommendations(
    ...
    if (requirement.status === 'completed') {
      recommendations.push(
        '完了済みの要求です。変更には慎重な検討とロールバック計画が必要です'
      );
    }
    ...
    if (proposedChanges?.priority && proposedChanges.priority !== requirement.priority) {
      recommendations.push(
        '優先度の変更は、開発スケジュールに影響を与える可能性があります。関係者への通知を忘れずに'
      );
    }
    ...
    if (proposedChanges?.dependencies) {
      recommendations.push(
        '依存関係の変更は、実装順序に影響を与えます。プロジェクト計画の見直しを推奨します'
      );
    }
   ...
   * 依存関係グラフを取得（可視化用）
   */
  async getDependencyGraph(requirementId: string): Promise<{
    nodes: Array<{ id: string; title: string; status: string }>;
    edges: Array<{ from: string; to: string }>;
  }> {
    const visited = new Set<string>();
    const nodes: Array<{ id: string; title: string; status: string }> = [];
