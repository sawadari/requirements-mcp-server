...
export class OntologyLoader {
   ...
   */
  static async loadFromFile(filePath: string): Promise<OntologyManager> {
    try {
      logger.info('Loading ontology schema', { filePath });
      const content = await fs.readFile(filePath, 'utf-8');
      const schema = JSON.parse(content) as OntologySchema;
      ...
      if (!validation.valid) {
        ...
        throw new Error(
          `Invalid ontology schema: ${validation.errors.map((e) => e.message).join(', ')}`
        );
      }
   ...
   */
  static async loadDefault(): Promise<OntologyManager> {
   ...
   * 環境変数で指定されたスキーマをロード
   */
  static async loadFromEnvironment(): Promise<OntologyManager> {
   ...
   * スキーマをファイルに保存
   */
  static async saveToFile(manager: OntologyManager, filePath: string): Promise<void> {
    try {
   ...
   * 利用可能なオントロジースキーマをリスト
   */
  static async listAvailable(directory: string = './config'): Promise<string[]> {
    try {
      const files = await fs.readdir(directory);
      const ontologyFiles = files.filter(
        (f) => f.startsWith('ontology-') && f.endsWith('.json')
      );
