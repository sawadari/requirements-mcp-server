...
import { OntologyLoader } from '../ontology/index.js';
/**
 * ルール設定ローダー
 */
export class RuleConfigLoader {
     ...
     */
    static async loadRules(configPath) {
...
export class ValidationEngine {
    config;
    ontologyManager;
    constructor(config, ontologyManager) {
        this.config = config;
     ...
     */
    static async create(configPath, ontologyPath) {
     ...
     * 単一要求を検証
     */
    async validateRequirement(req, allRequirements, options = {}) {
        const violations = [];
        // NLP指標を計算・更新（オプション）
        ...
        const hierarchyViolations = StructureValidationEngine.validateRequirement(req, allRequirements, hierarchyRules);
        violations.push(...hierarchyViolations);
        // B: グラフヘルス（要求単位: B2-B5）
        const graphRules = this.config.rules.graph_health.filter(r => r.enabled && r.id !== 'B1');
        const graphViolations = StructureValidationEngine.validateRequirement(req, allRequirements, graphRules);
        violations.push(...graphViolations);
        // C: 抽象度ルール
        const abstractionRules = this.config.rules.abstraction.filter(r => r.enabled);
        const abstractionViolations = AbstractionValidator.validateAll(req, allRequirements, abstractionRules);
        ...
        const meceViolations = MECEValidator.validateAll(req, allRequirements, meceRules);
        ...
        if (options.useLLM && LLMEvaluator.isAvailable()) {
            enhancedViolations = await Promise.all(violations.map(v => LLMEvaluator.enhanceViolationWithLLM(req, v, allRequirements)));
        }
        // スコア計算
     ...
     * 全要求を検証
     */
    async validateAll(allRequirements, options = {}) {
        ...
        const globalViolations = new Map();
        if (dagRule) {
     ...
     */
    generateReport(results, allRequirements) {
        ...
        const avgScore = Array.from(results.values()).reduce((sum, r) => sum + (r.score || 0), 0) /
            totalRequirements;
        let report = `# 要求検証レポート
