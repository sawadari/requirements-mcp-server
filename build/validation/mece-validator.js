...
export class TextSimilarity {
     ...
     * 2つの要求の類似度を計算
     */
    static calculateRequirementSimilarity(req1, req2) {
...
export class AbstractionValidator {
     ...
     * C1: 兄弟要求の抽象度一貫性チェック
     */
    static validateSiblingConsistency(req, allRequirements, rule) {
        const violations = [];
        if (!req.refines || req.refines.length === 0) {
            return violations; // 親がない場合はスキップ
        }
        // 同じ親を持つ兄弟要求を取得
        const siblings = Array.from(allRequirements.values()).filter(r => r.id !== req.id &&
     ...
     * C2: 親子間の具体化度差チェック
     */
    static validateParentChildConcreteness(req, allRequirements, rule) {
        const violations = [];
        if (!req.refines || req.refines.length === 0) {
            return violations;
        }
     ...
     * C3: 抽象度スコアの範囲チェック
     */
    static validateScoreRange(req, rule) {
        const violations = [];
     ...
     * 抽象度全体の検証
     */
    static validateAll(req, allRequirements, rules) {
        const violations = [];
...
export class MECEValidator {
     ...
     * D1: 兄弟要求間の重複検出
     */
    static detectSiblingOverlap(req, allRequirements, rule) {
        const violations = [];
        if (!req.refines || req.refines.length === 0) {
            return violations;
        }
     ...
     */
    static validateParentCoverage(req, allRequirements, rule) {
        const violations = [];
        // この要求が親の場合のみチェック
     ...
     * D3: 細分化の妥当性チェック
     */
    static validateFragmentation(req, allRequirements, rule) {
        const violations = [];
        const children = Array.from(allRequirements.values()).filter(r => r.refines?.includes(req.id));
        if (children.length === 0) {
            return violations;
        ...
        // 子要求が小さすぎる場合
        const tooSmallChildren = children.filter(c => (c.length_tokens || 0) < minTokensPerChild);
