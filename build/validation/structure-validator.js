...
export class HierarchyValidator {
     ...
     * オントロジーマネージャーを設定
     */
    static setOntologyManager(manager) {
        HierarchyValidator.ontologyManager = manager;
    }
    /**
     * A1: レベル間の関係制約チェック（オントロジー対応）
     */
    static validateParentChildTypes(req, allRequirements, rule) {
        const violations = [];
        ...
        for (const parentId of req.refines) {
            ...
            const isAllowed = allowedPairs.some((pair) => pair.parent === parent.type && pair.child === req.type);
            if (!isAllowed) {
                violations.push({
                    id: `${rule.id}-${req.id}-${parentId}`,
                    requirementId: req.id,
     ...
     * オントロジーから許可ペアを構築
     */
    static buildAllowedPairsFromOntology() {
     ...
     * A2: 親要求の存在チェック
     */
    static validateParentExistence(req, rule) {
        const violations = [];
     ...
     */
    static detectCycles(req, allRequirements, rule) {
        const violations = [];
     ...
     * A4: 最大階層深度チェック
     */
    static validateMaxDepth(req, allRequirements, rule) {
        const violations = [];
        const maxDepth = rule.parameters?.maxDepth || 3;
        const getDepth = (currentId, visited = new Set()) => {
            if (visited.has(currentId))
                return 0; // 循環参照対策
            visited.add(currentId);
            const current = allRequirements.get(currentId);
...
export class GraphHealthValidator {
     ...
     */
    static validateDAG(allRequirements, rule) {
        const violations = [];
        const visited = new Set();
        const recStack = new Set();
        const hasCycle = (currentId) => {
     ...
     * B2: ファンアウト制限（子要求数の上限）
     */
    static validateFanOut(req, allRequirements, rule) {
        const violations = [];
        const maxChildren = rule.parameters?.maxChildren || 10;
        // この要求を親（refines）とする子要求を数える
        const children = Array.from(allRequirements.values()).filter(r => r.refines?.includes(req.id));
     ...
     * B3: 孤立要求の検出
     */
    static detectOrphans(req, allRequirements, rule) {
        const violations = [];
        const hasIncoming = Array.from(allRequirements.values()).some(r => r.refines?.includes(req.id) ||
            r.depends_on?.includes(req.id) ||
            r.conflicts_with?.includes(req.id) ||
            r.duplicates?.includes(req.id));
        const hasOutgoing = (req.refines && req.refines.length > 0) ||
     ...
     * B4: 競合関係の双方向対称性チェック
     */
    static validateConflictSymmetry(req, allRequirements, rule) {
        const violations = [];
     ...
     */
    static validateDuplicateCandidates(req, allRequirements, rule) {
        const violations = [];
        if (!req.duplicates)
            return violations;
...
export class StructureValidationEngine {
     ...
     */
    static validateRequirement(req, allRequirements, rules) {
        const violations = [];
     ...
     */
    static validateAll(allRequirements, rules) {
        const violationsByReq = new Map();
        // グラフ全体のチェック（B1: DAG）
