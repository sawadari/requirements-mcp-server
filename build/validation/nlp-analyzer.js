...
export class NLPAnalyzer {
     ...
     */
    static calculateNumericDensity(text) {
     ...
     */
    static estimateAtomicityScore(text) {
     ...
     */
    static estimateAbstractionScore(text) {
...
export class QualityStyleValidator {
     ...
     * E1: 曖昧な表現の検出
     */
    static detectVagueTerms(req, rule) {
        const violations = [];
     ...
     * E2: 受動態の検出
     */
    static detectPassiveVoice(req, rule) {
        const violations = [];
     ...
     */
    static validateActorPresence(req, rule) {
        const violations = [];
     ...
     * E4: 長さの妥当性チェック
     */
    static validateLength(req, rule) {
        const violations = [];
        const minTokens = rule.parameters?.minTokens || 10;
        const maxTokens = rule.parameters?.maxTokens || 200;
        const tokens = req.length_tokens || NLPAnalyzer.calculateTokens(`${req.title} ${req.description} ${req.rationale || ''}`);
        if (tokens < minTokens) {
            violations.push({
     ...
     */
    static validateAtomicity(req, rule) {
        const violations = [];
        const minScore = rule.parameters?.minAtomicityScore || 0.7;
        const score = req.atomicity_score ?? NLPAnalyzer.estimateAtomicityScore(`${req.title} ${req.description}`);
        if (score < minScore) {
            violations.push({
                id: `${rule.id}-${req.id}`,
                requirementId: req.id,
