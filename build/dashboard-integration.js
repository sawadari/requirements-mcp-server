...
export class DashboardIntegration {
    dataDir;
    dashboardDir;
    constructor(dataDir = './data', dashboardDir = './dashboard') {
     ...
     */
    async generateMetrics(requirements) {
        const byStatus = {};
        const byPriority = {};
        const byCategory = {};
     ...
     * 依存関係の健全性を分析
     */
    analyzeDependencyHealth(requirements) {
     ...
     * ダッシュボードデータをファイルに保存
     */
    async saveDashboardData(metrics, widgets) {
        // ダッシュボードディレクトリが存在しない場合は作成
        await fs.mkdir(this.dashboardDir, { recursive: true });
        // メトリクスを保存
        await fs.writeFile(join(this.dashboardDir, 'metrics.json'), JSON.stringify(metrics, null, 2), 'utf-8');
        // ウィジェットを保存
        await fs.writeFile(join(this.dashboardDir, 'widgets.json'), JSON.stringify(widgets, null, 2), 'utf-8');
        // タイムスタンプを保存
        await fs.writeFile(join(this.dashboardDir, 'last-updated.json'), JSON.stringify({ timestamp: new Date().toISOString() }, null, 2), 'utf-8');
        console.log(`📊 Dashboard data saved to ${this.dashboardDir}`);
    }
    /**
     * 完全なダッシュボードデータを生成・保存
     */
    async generateAndSaveDashboard(requirements) {
     ...
     * 差分更新: 変更があった要求のみ更新
     */
    async updateDashboardIncremental(changedRequirements, allRequirements) {
 ...
 */
export function createDashboardIntegration(dataDir, dashboardDir) {
