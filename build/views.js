...
export class ViewExporter {
     ...
     * フィルタを適用して要求を取得
     */
    async getFilteredRequirements(viewDef) {
        let requirements = await this.storage.getAllRequirements();
        if (viewDef.filters) {
            if (viewDef.filters.category) {
                requirements = requirements.filter((r) => viewDef.filters.category.includes(r.category));
            }
            if (viewDef.filters.status) {
                requirements = requirements.filter((r) => viewDef.filters.status.includes(r.status));
            }
            if (viewDef.filters.priority) {
                requirements = requirements.filter((r) => viewDef.filters.priority.includes(r.priority));
            }
        }
     ...
     * リスト形式でエクスポート
     */
    exportList(requirements, viewDef) {
     ...
     */
    exportListAsMarkdown(requirements, columns, viewDef) {
     ...
     * CSV形式でリストをエクスポート
     */
    exportListAsCSV(requirements, columns) {
        const headers = columns.map(this.getColumnLabel);
     ...
     * HTML形式でリストをエクスポート
     */
    exportListAsHTML(requirements, columns, viewDef) {
...
`;
     ...
     * マトリックス形式でエクスポート
     */
    async exportMatrix(viewKey, viewDef) {
     ...
     */
    async exportStakeholderSystemMatrix(viewDef) {
        const stakeholderReqs = (await this.storage.getAllRequirements()).filter((r) => r.category === 'ステークホルダ要求').sort((a, b) => a.id.localeCompare(b.id));
        const systemReqs = (await this.storage.getAllRequirements()).filter((r) => r.category === 'システム要求').sort((a, b) => a.id.localeCompare(b.id));
        let output = `# ${viewDef.name}\n\n`;
        output += `${viewDef.description}\n\n`;
        output += `生成日時: ${new Date().toLocaleString('ja-JP')}\n\n`;
        output += `---\n\n`;
     ...
     * システム要求-機能要求マトリックス
     */
    async exportSystemFunctionalMatrix(viewDef) {
        const systemReqs = (await this.storage.getAllRequirements()).filter((r) => r.category === 'システム要求').sort((a, b) => a.id.localeCompare(b.id));
        const functionalReqs = (await this.storage.getAllRequirements()).filter((r) => r.category === 'システム機能要求').sort((a, b) => a.id.localeCompare(b.id));
        let output = `# ${viewDef.name}\n\n`;
        output += `${viewDef.description}\n\n`;
        output += `生成日時: ${new Date().toLocaleString('ja-JP')}\n\n`;
        output += `---\n\n`;
     ...
     * 値をフォーマット
     */
    formatValue(req, column) {
        const value = req[column];
     ...
     */
    async exportAllViews(outputDir = './views') {
