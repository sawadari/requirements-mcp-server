...
export class ChangeEngine {
     ...
     */
    preview(changeSet, requirements) {
     ...
     */
    async apply(changeSet, requirements) {
        // 1. 元の状態をディープコピー（ロールバック用）
        const original = JSON.parse(JSON.stringify(requirements));
        const modified = JSON.parse(JSON.stringify(requirements));
        const errors = [];
     ...
     */
    async rollbackAppliedChanges(appliedChanges, current, original) {
     ...
     * ChangeSetをロールバック
     */
    async rollback(changeSet, requirements) {
     ...
     * 個別のChangeを適用
     */
    async applyChange(change, requirements) {
        try {
            switch (change.op) {
                case 'split':
     ...
     * 分割操作の適用
     */
    applySplit(change, requirements) {
     ...
     * 統合操作の適用
     */
    applyMerge(change, requirements) {
     ...
     * リンク再配線の適用
     */
    applyRewire(change, requirements) {
     ...
     * テキスト書き換えの適用
     */
    applyRewrite(change, requirements) {
        const targetId = Array.isArray(change.target) ? change.target[0] : change.target;
        const target = requirements[targetId];
        if (!target) {
            return { success: false, error: `要求 ${targetId} が見つかりません` };
     ...
     * エイリアス設定の適用
     */
    applyAlias(change, requirements) {
        const targetId = Array.isArray(change.target) ? change.target[0] : change.target;
        const target = requirements[targetId];
        if (!target) {
            return { success: false, error: `要求 ${targetId} が見つかりません` };
     ...
     * 循環切断の適用
     */
    applyBreakCycle(change, requirements) {
