...
export class FixExecutor {
    ...
    async execute(requirements, validate) {
        ...
        try {
            while (iteration < this.policy.stopping.max_iterations) {
                ...
                // Strict違反のみをチェック
                const strictViolations = violations.filter(v => this.isStrictViolation(v.code || v.ruleName));
    ...
    canAutoApply(changeSet) {
        ...
        return changeSet.changes.every(change => {
            ...
            const rule = this.policy.rules.find(r => r.whenViolation === violation || r.id === violation);
            if (!rule) {
     ...
     * 修正プランのプレビュー（適用せずに確認）
     */
    async previewFixes(requirements, validate) {
     ...
     * 特定のChangeSetsのみを適用
     */
    async applySelected(changeSetIds, requirements, allChangeSets) {
        let currentReqs = { ...requirements };
        const appliedChangeSets = [];
        const fixedViolations = new Set();
     ...
     * ロールバック
     */
    async rollbackAll(changeSets, requirements) {
        let currentReqs = { ...requirements };
        const errors = [];
        // 逆順でロールバック
