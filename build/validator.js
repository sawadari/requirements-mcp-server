...
export class RequirementValidator {
    ...
    buildContext(requirement, allRequirements) {
        ...
        // 下位要求を取得
        const childRequirements = allRequirements.filter((r) => r.dependencies.includes(requirement.id) || r.parentId === requirement.id);
        // 兄弟要求を取得（同じ親を持つ要求）
        const siblingRequirements = allRequirements.filter((r) => {
            if (r.id === requirement.id)
                return false;
            // 同じ親IDを持つ
            if (requirement.parentId && r.parentId === requirement.parentId)
                return true;
            // 同じ依存関係を持つ
            const sharedParents = r.dependencies.filter((dep) => requirement.dependencies.includes(dep));
    ...
    // ルール1: 上位要求との整合性チェック
    checkParentConsistency(context) {
        const results = [];
    ...
    checkDecomposition(context) {
        ...
        if (config.checkOverlap) {
            for (const sibling of siblingRequirements) {
                const similarity = this.calculateSimilarity(requirement.title, sibling.title);
    ...
    inferType(requirement) {
        if (requirement.type) {
            // system_functionalをfunctionalに正規化
            if (requirement.type === 'system_functional')
                return 'functional';
        ...
        // IDから推測
        if (requirement.id.startsWith('STK-'))
            return 'stakeholder';
        if (requirement.id.startsWith('SYS-'))
            return 'system';
        if (requirement.id.startsWith('FUNC-'))
            return 'functional';
        return 'stakeholder'; // デフォルト
    }
    // ヘルパーメソッド: 階層関係の検証
    isValidHierarchy(parentType, childType) {
        if (parentType === 'stakeholder' && childType === 'system')
            return true;
        if (parentType === 'system' && childType === 'functional')
            return true;
        return false;
    }
    // ヘルパーメソッド: 期待される子要求の種類
    getExpectedChildType(parentType) {
        if (parentType === 'stakeholder')
            return 'システム要求';
     ...
     * - 理由に必要なキーワードが含まれているか
     */
    checkRationaleConsistency(context) {
        const results = [];
        const { requirement } = context;
        ...
        if (config.checkKeywordPresence && config.requiredElements) {
            ...
            const missingKeywords = requiredKeywords.filter((keyword) => !requirement.rationale.includes(keyword));
