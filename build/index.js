...
class RequirementsMCPServer {
    ...
    async handleAddRequirement(args) {
    ...
    async handleGetRequirement(args) {
    ...
    async handleUpdateRequirement(args) {
    ...
    async handleDeleteRequirement(args) {
    ...
    async handleSearchRequirements(args) {
    ...
    async handleAnalyzeImpact(args) {
        ...
        const analysis = await this.analyzer.analyzeImpact(params.id, params.proposedChanges);
        return {
            content: [
                {
                    type: 'text',
    ...
    async handleProposeChange(args) {
        const params = ProposeChangeSchema.parse(args);
        // 影響分析を実行
        const proposedChangesObj = {};
        for (const change of params.proposedChanges) {
            proposedChangesObj[change.field] = change.proposedValue;
        }
        const impactAnalysis = await this.analyzer.analyzeImpact(params.targetRequirementId, proposedChangesObj);
        // 変更提案を作成
        const proposal = {
            id: `PROP-${Date.now()}`,
    ...
    async handleValidateRequirement(args) {
        ...
        const result = await this.validationEngine.validateRequirement(requirement, requirementsMap, {
    ...
    async handleValidateAllRequirements(args) {
        ...
        const allRequirements = await this.storage.getAllRequirements();
        const requirementsMap = new Map(allRequirements.map(r => [r.id, r]));
        const results = await this.validationEngine.validateAll(requirementsMap, {
            useLLM: params.useLLM ?? false,
            updateMetrics: params.updateMetrics ?? true,
        });
        // 結果をキャッシュ
    ...
    async handleGetValidationReport(args) {
    ...
    async handleLoadPolicy(args) {
        ...
        try {
            ...
            this.currentPolicy = parse(content);
            // FixExecutorを初期化
    ...
    async handleRollbackFixes(args) {
