...
export class StepExecutor {
    ...
    batchTools;
    anthropic = null;
    constructor(storage, validationTools, batchTools) {
        this.storage = storage;
        this.validationTools = validationTools;
        this.batchTools = batchTools;
     ...
     * AI生成ステップ
     */
    async executeAIGeneration(step, context) {
        if (!this.anthropic) {
            return {
                stepId: step.id,
                success: false,
     ...
     * MCP呼び出しステップ
     */
    async executeMCPCall(step, context) {
     ...
     * 確認ステップ（レポート生成）
     */
    async executeConfirmation(step, context) {
     ...
     * 依存関係チェック
     */
    checkDependencies(step, context) {
     ...
     * システムプロンプト構築
     */
    buildSystemPrompt(step, context) {
...
[
  ...
  ...
     ...
     * 生成プロンプト構築
     */
    buildGenerationPrompt(step, context) {
        return `${step.description}\n\nユーザーの意図: ${context.intent.rawMessage}`;
    }
    /**
     * ツールパラメータ構築
     */
    buildToolParams(step, context) {
        return {};
    }
    /**
     * 生成された要求を抽出
    ...
    generateReport(context) {
        ...
        if (validationResults.length > 0) {
            const allValid = validationResults.every((r) => {
                return Array.isArray(r) ? r.every((x) => x.isValid) : r.isValid;
            });
            report += `\n### 妥当性チェック\n`;
