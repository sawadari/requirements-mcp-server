...
async function validate() {
  ...
  // FUNC-002を取得
  const requirement = await storage.getRequirement('FUNC-002');
  if (!requirement) {
    console.log('FUNC-002が見つかりません');
    return;
  }
  ...
  // 結果を表示
  console.log('=== FUNC-002: ' + requirement.title + ' の検証結果 ===\n');
  ...
  console.log('  Refines: ' + (requirement.refines?.join(', ') || 'なし'));
